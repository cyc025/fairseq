...
for ((i=startlen; i<=upperlim; i++)); do
    for ((j=1; j<=10; j++)); do
        ...
        taskset --cpu-list 1 python examples/bart/summarize.py   --model-dir .   --model-file checkpoints/bart.base/model.pt   --src ~/fairseq_cnn_data/cnn_cln/toy_test.source   --out ~/fairseq_cnn_data/cnn_cln/test.hypo --sampling-topp 0.1;
