...
class TranslationLevenshteinTask(TranslationTask):
    ...
    def inject_noise(self, target_tokens, mask_distribution=None):
        ...
        def _random_mask(target_tokens):
            ...
            def predict_start_uniform_end(): # 23.49
            ...
            def predict_start_uniform_end_relaxed_v4(): # 24.86
                """ DyMask-v1 (predict start, uniform end positions) """
                nonlocal target_length
            ...
            def predict_start_uniform_end_relaxed_v2(): # 24.83
                """ DyMask-v1 (predict start, uniform end positions) """
                nonlocal target_length
            ...
            def predict_start_uniform_end_relaxed(): # 24.61 / 21.43
                """ DyMask-v1 (predict start, uniform end positions) """
                nonlocal target_length
            ...
            def uniform_start_end(): # 19.54 / 24.73
                """ uniform start and end """
                nonlocal target_length
                end_ratio = target_length.clone().uniform_(0.8,1.0)
                start_ratio = target_length.clone().uniform_() #target_length.clone().uniform_(0.,0.8)
                return map_single_segment(start_ratio,end_ratio)

            def fixed_start_variable_end(): # x / 25.95
            ...
            def variable_start_variable_end(): # x / 26.38
                """ DyMask (predict fixed start, variable end positions) """
                nonlocal target_length
                end_ratio = mask_distribution
                start_ratio = target_length.clone().uniform_(0.0,0.2)
                # logger.info(mask_distribution)
                return map_single_segment(start_ratio,end_ratio)

            def variable_start_variable_end_v2(): # x / 25.69
                """ DyMask (predict fixed start, variable end positions) """
                nonlocal target_length
                end_ratio = mask_distribution
                start_ratio = target_length.clone().uniform_(0.0,0.1)
                # logger.info(mask_distribution)
                return map_single_segment(start_ratio,end_ratio)
            ...
            def fixed_start_fixed_end_v2(): # x / 27.25
                """ DyMask (predict fixed start, variable end positions) """
                nonlocal target_length
                end_ratio = target_length.clone().uniform_(1.0,1.0)
                start_ratio = target_length.clone().uniform_(0.0,0.0)
            ...
            def uniform_original(): # 19.97 / 28.85
                """ uniform original """
            ...
            def uniform_original_fixed(): # x / 28.01
                """ uniform original """
                nonlocal target_length, target_rank
                end_ratio = mask_distribution
                # convert to length-wise
            ...
            def uniform_original_no_mask(): # x / ?
                """ uniform original """
                nonlocal target_length, target_rank
                end_ratio = target_length.clone().uniform_(1.,1.)
                # convert to length-wise
                target_length = target_length * end_ratio
                target_length = target_length + 1  # make sure to mask at least one token.
                target_cutoff = new_arange(target_rank) < target_length[:, None].long()
            ...
            def multi_segment_flip(): # x / 22.64
                ### DyMask-v3: multi-segment, self-supervising masking mechanism
                nonlocal target_rank, target_length
                import numpy as np
                import random
