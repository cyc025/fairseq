...
# python train.py --task language_modeling data-bin/wikitext-2 --decoder-normalize-before --save-dir checkpoints/transformer_wikitext-2 --arch transformer_lm --share-decoder-input-output-embed --dropout 0.1 --optimizer adam --adam-betas '(0.9, 0.98)' --weight-decay 0.01 --clip-norm 0.0 --lr 0.00005 --lr-scheduler inverse_sqrt --warmup-updates 4000 --warmup-init-lr 1e-07 --tokens-per-sample 512 --sample-break-mode none --max-tokens 2048 --update-freq 16 --fp16 --batch-size 1024 \
# --decoder-embed-dim 50 --decoder-layers 2 --decoder-attention-heads 2  --max-update 8000 --no-epoch-checkpoints --keep-best-checkpoints 1 --tensorboard-logdir tensorboard
...
# run all
result_list = []
for i in range(1,len(params_list),5):
    tups = params_list[i]
